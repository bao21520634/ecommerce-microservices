    ...
    implements IEventPublisher, OnModuleDestroy, OnModuleInit, IMessageSource
{
    ...
    constructor(
        ...
        if (esStreamConfig.type === 'event-store') {
            ...
            throw new Error(
                'Event store type is not supported - (event-tore.ts)',
            );
        }
    ...
    subscribeToCatchupSubscription(
        ...
        try {
            return this.eventStore.getClient().subscribeToStreamFrom(
                ...
                (sub, reason, error) =>
                    this.onDropped(
                        sub as ExtendedCatchUpSubscription,
                        reason,
                        error,
                    ),
        ...
        try {
            const resolved = (await this.eventStore
                ...
                .subscribeToStream(
                    ...
                    (sub, reason, error) =>
                        this.onDropped(
                            sub as ExtendedVolatileSubscription,
                            reason,
                            error,
                        ),
            ...
            this.logger.log(
                'Volatile processing of EventStore events started!',
            );
    ...
    get allVolatileSubscriptionsLive(): boolean {
        const initialized =
            this.volatileSubscriptions.length ===
            this.volatileSubscriptionsCount;
        return (
            initialized &&
    ...
    get allPersistentSubscriptionsLive(): boolean {
        const initialized =
            this.persistentSubscriptions.length ===
            this.persistentSubscriptionsCount;
        return (
            initialized &&
