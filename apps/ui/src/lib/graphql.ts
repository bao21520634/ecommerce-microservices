...
export async function executeGraphQL<Result, Variables>(
	...
	invariant(process.env.NEXT_PUBLIC_EC_API_URL, "Missing NEXT_PUBLIC_EC_API_URL env variable");
	...
	const response = withAuth
		? await (await getServerAuthClient()).fetchWithAuth(process.env.NEXT_PUBLIC_EC_API_URL, input)
		: await fetch(process.env.NEXT_PUBLIC_EC_API_URL, input);
